...
\begin{theorem}\label{def1}
    ...
    \begin{itemize}
        ...
        \item $q_0 \in Q$ \emph{je} počáteční stav\emph{,} $Z_0 \in \Gamma$ \emph{je} startovací symbol zásobníku \emph{a $F \subseteq Q$ je množina} koncových stavů.
    \end{itemize}
\end{theorem}
...
\begin{theorem}\label{def2}
    ...
    Množina $L(A) = \{w \mid w \textrm{ je přijat ZA } A \} \subseteq \Sigma^\ast$ je} jazyk přijímaný ZA $A$.
\end{theorem}
\begin{sentence}
