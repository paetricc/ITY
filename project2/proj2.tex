...
\begin{theorem}\label{def2}
    \emph{Řetězec $w$ nad abecedou $\Sigma$ je přijat ZA} $A$~jestliže $(q_0, w, Z_0) \overset{\ast}{\underset{A}{\vdash}} (q_F, \epsilon, \gamma)$ pro nějaké $\gamma \in \Gamma^\ast$ a $q_F \in F$.
    Množina $L(A) = \{w \mid w \textrm{ je přijat ZA } A \} \subseteq \Sigma^\ast$ je \emph{jazyk přijímaný ZA} $A$.
\end{theorem}
\begin{sentence}
